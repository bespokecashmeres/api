...
const getDeleteOrStatusStatuses = async (colourId, checkStatus = false) => {
  const isYarn = await yarnFindRecord({
    colourId: new ObjectId(colourId),
...
exports.deleteController = async (req, res, next) => {
  ...
  if (isYarn) {
    throw {
      ...
      message: res.__(
        serverResponseMessage.PLEASE_DELETE_YARN_TO_DELETE_COLOUR
      ),
    };
  }
...
exports.statusController = async (req, res, next) => {
  ...
  if (isYarn) {
    throw {
      code: httpStatusCodes.BAD_REQUEST,
      message: res.__(
        serverResponseMessage.PLEASE_DISABLE_YARN_TO_DISABLE_COLOUR
      ),
    };
  }
