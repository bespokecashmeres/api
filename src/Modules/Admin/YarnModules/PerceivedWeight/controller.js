...
const getDeleteOrStatusStatuses = async (
  perceivedWeightId,
  checkStatus = false
) => {
  const isYarn = await yarnFindRecord({
    perceivedWeightId: new ObjectId(perceivedWeightId),
...
exports.deleteController = async (req, res, next) => {
  ...
  if (isYarn) {
    throw {
      ...
      message: res.__(
        serverResponseMessage.PLEASE_DELETE_YARN_TO_DELETE_PERCEIVED_WEIGHT
      ),
    };
  }
...
exports.statusController = async (req, res, next) => {
  ...
  if (isYarn) {
    throw {
      code: httpStatusCodes.BAD_REQUEST,
      message: res.__(
        serverResponseMessage.PLEASE_DISABLE_YARN_TO_DISABLE_PERCEIVED_WEIGHT
      ),
    };
  }
