...
const getDeleteOrStatusStatuses = async (
  ...
  const isYarn = await yarnFindRecord({
    seasonalityId: new ObjectId(seasonalityId),
...
exports.deleteController = async (req, res, next) => {
  ...
  if (isYarn) {
    throw {
      ...
      message: res.__(
        serverResponseMessage.PLEASE_DELETE_YARN_TO_DELETE_SEASONALITY
      ),
    };
  }
...
exports.statusController = async (req, res, next) => {
  ...
  if (isYarn) {
    throw {
      code: httpStatusCodes.BAD_REQUEST,
      message: res.__(
        serverResponseMessage.PLEASE_DISABLE_YARN_TO_DISABLE_SEASONALITY
      ),
    };
  }
