...
exports.createController = async (req, res) => {
  ...
  if (image) {
    try {
      const imagePath = await uploadToS3(image, "main-category");
...
exports.updateController = async (req, res, next) => {
  ...
  try {
    ...
    if (image) {
      try {
        const path = await uploadToS3(image, "main-category");
        req.body.image = path;
  ...
  return res
    ...
    .json(
      success(
        ...
        httpResponses.SUCCESS,
        res.__(serverResponseMessage.RECORD_FETCHED),
        await getMainCategoriesForDropdown(req.body, acceptLanguage)
      )
    );
};
