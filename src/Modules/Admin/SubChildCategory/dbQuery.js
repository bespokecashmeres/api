...
module.exports.getPaginationData = async (qData) => {
  ...
  const projectFields = {
    ...
    status: 1,
    childCategory: {
      $ifNull: [`$childCategoryInfo.name.${language}`, ""],
    },
    subCategory: {
      $ifNull: [`$subCategoryInfo.name.${language}`, ""],
    },
    mainCategory: {
  ...
  const pipeline = [
    ...
    },
    {
      $lookup: {
        ...
        foreignField: "_id",
        as: "childCategoryInfo",
      },
    },
    {
      $unwind: { path: "$childCategoryInfo", preserveNullAndEmptyArrays: true },
    },
...
module.exports.getSubChildCategoriesForDropdown = async (
  { genderId, mainCategoryId, subCategoryId, childCategoryId },
  language
) => {
  const pipeline = [
    {
      $match: {
        ...
        childCategoryId: new ObjectId(childCategoryId)
      },
    },
    { $sort: { rowOrder: 1 } },
    {
