...
exports.createController = async (req, res) => {
  ...
  if (image) {
    try {
      const imagePath = await uploadToS3(image, "sub-child-category");
...
exports.updateController = async (req, res, next) => {
  ...
  try {
    ...
    if (image) {
      try {
        const path = await uploadToS3(image, "sub-child-category");
        req.body.image = path;
...
exports.dropdownOptionsController = async (req, res, next) => {
  ...
  return res.status(httpStatusCodes.SUCCESS).json(
      success(
          ...
          httpResponses.SUCCESS,
          res.__(serverResponseMessage.RECORD_FETCHED),
          await getSubChildCategoriesForDropdown(req.body, acceptLanguage)
      )
  );
};
